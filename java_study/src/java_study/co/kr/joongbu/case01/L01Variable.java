...
public class L01Variable {
	...
	System.out.println(obj == obj2); // 객채는 새로 생성될때마다 완전새거라서 false 나온다 
	
	//== 동등 비교가비교하는 것 
	//1. 객체가 같은 것인지 비교 (참조형은 선언할 때마다 새로운 객체를 만들기 때문에 무조건 false -> 참조형 (객체) 
	//2. 값이 같은지 비교 (값은 기본형 -> 수 )  -> 기본형 
	//3. 참조형을 비교할땐 주소를 비교한다 !! -> 참조형 (완전히 같은 객체인지를 비교한다) 
	
	System.out.println(20 == 20.0);
	System.out.println(new String ("안녕") == new String("안녕")); //참조형 객체는 선언할때마다 새로 생기니까 무조건 false 가 나오게 된다 
	
	//예외가 존재한다 !!!!
	System.out.println("안녕"=="안녕"); 
	//이거 true 가 뜬다 (참조형중에 String 만 예외임 !!) 문자열을 리터럴하게 선언 시 like 기본형 새로운 data 를 만들지 않고 기존의 객체를 찾아서 참조한다 
	
	//참조형을 비교연산 하는 방법 ?? Object.equals(obj) 서로 다른 두 객체가 같으려면 ?? (일란성 쌍둥이는 같다 : 유전정보가 같다 ! -> !! 필드가 완전히 같아야한다 !!(주관적으로 필드의 범위 지정 가능 )) => 재정의해야가능 
	//String 은 equals 를 재정의 하고있다. 
	//리터럴하게 선언된 문자열(String) 도 참조형 데이터이다 = 동등비교를 권장하지 않는다 (참조형도 동등비교 할 수 있나요 ? => 안됩니다 ! => 왜 안됩니까 ? 문자열은 비교하던데 => 권장하지 않아요 ~) 
